...
let CalcEngine = new CalculationEngine()
let inline input x = CalcEngine.AddInput x
let inline output nodes f = CalcEngine.AddOutput(nodes, f)

// static methods of async functions to apply at output nodes
let add2(x1, x2) = 
    async { 
        do! Async.Sleep 500
        return x1 + x2
