...
type CalculationEngine(calculationHandler : ICalculationHandler) as this = 
    ...
    // two input/output methods instead of using an F# optional parameter
    // otherwise an F# option would be exposed in the API
...
//    member this.AddOutput(dependentNodes : 'N, nodeFunction : 'T -> 'U) =
